...
DFLASH currently rejects grammar-constrained requests, including JSON schema,
regex, EBNF, structural-tag, `tool_choice="required"`, and named tool choices.
Parsed tool calls remain available with unconstrained `tool_choice="auto"`.
Serve without DFLASH when the API must enforce a grammar or tool choice.
