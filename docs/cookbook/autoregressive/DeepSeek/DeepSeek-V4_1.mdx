---
title: DeepSeek-V4.1
description: "Deploy DeepSeek-V4.1 Flash with SGLang — launch recipes, feature compatibility, and tuning notes for GB300, H200, B200, B300, MI350X and 8×V100."
tag: NEW
---
...
Volta has no FlashMLA / MXFP4-on-SM90 path, so this shape is a source build (`scripts/serve_dsv41_v100.sh`) rather than the preview Docker cells above. The Deploy panel does not emit it.

Best measured single-user setup on 8×V100-SXM2-32GB: **DSpark on**, **sticky last-sequence prompt cache** (radix stays off), **256k context**, chunked prefill 2048, `np=1`. Coding-trace wall is about **8 tok/s** at temperature 0 (accept length ~4.5–6, α ~5.9). Open-ended chat at the server default temperature 1 is closer to **3 tok/s** (accept length ~2). Prefill is about **1.0–1.2k tok/s** at 8k. Do not tune `--speculative-dspark-block-size`; the checkpoint is γ=5.

<Accordion title="8×V100 launch (source)">

```bash Command
# Host Engram + 1G hugepages on the GPU NUMA node, then:
...
The script pins the rest: `--speculative-algorithm DSPARK` against the same checkpoint, `--attention-backend dsv4`, `--moe-runner-backend marlin`, `--mem-fraction-static 0.88`, spill 13 GiB / landing 36, `--disable-radix-cache`, `--disable-prefill-cuda-graph`, `--max-running-requests 1`, `--language-model-only`, Tree NCCL, `SGLANG_ENABLE_DSV41_ENGRAM_HOST_TABLE=1`.

<table style={{width: "100%", borderCollapse: "collapse", tableLayout: "fixed"}}>
  <thead>
    <tr style={{borderBottom: "2px solid #d55816"}}>
      <th style={{textAlign: "left", padding: "10px 12px", fontWeight: 700}}>Knob</th>
      <th style={{textAlign: "left", padding: "10px 12px", fontWeight: 700}}>Ship value</th>
  ...
  <tbody>
    <tr>
      ...
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.02)"}}>on (γ=5)</td>
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.02)"}}>Best token-generation on this box. `DSPARK=0` is greedy Tree (~5–8 tok/s cruise, no draft).</td>
    </tr>
    <tr>
      <td style={{padding: "9px 12px", fontWeight: 500, backgroundColor: "rgba(255,255,255,0.05)"}}>Prompt cache</td>
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.05)"}}>sticky last-seq</td>
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.05)"}}>Exact full-history continuation only. Partial prefix misses. Do not call `/health` (it drops the pin). Do not enable `--enable-streaming-session`.</td>
    </tr>
    <tr>
      <td style={{padding: "9px 12px", fontWeight: 500, backgroundColor: "rgba(255,255,255,0.02)"}}>Context</td>
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.02)"}}>262144</td>
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.02)"}}>CSA2 `kv_rows` scale with this. 512k has not been shown to leave ~300 MiB for the Engram MXFP8 unpack on 8k prefill.</td>
    </tr>
    <tr>
      <td style={{padding: "9px 12px", fontWeight: 500, backgroundColor: "rgba(255,255,255,0.05)"}}>Sampling</td>
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.05)"}}>temp=0 for code</td>
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.05)"}}>Highest DSpark accept. Reasoning evals on other cards used temp=1.0 / top_p=0.95 — expect the slower chat band here.</td>
    </tr>
  </tbody>
</table>

<Warning>
8×V100 is not the 15–20 tok/s product shape. NCCL wait on the hybrid NVLink mesh dominates the T=6 verify step. Treat the numbers above as the measured ceiling on this hardware, not a portable throughput claim.
</Warning>
