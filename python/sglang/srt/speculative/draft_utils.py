...
class DraftBackendFactory:
    ...
    def create_decode_backend(self):
        ...
        # Keyed off the draft config, not the configured backend string: a QSA
        # draft needs the sparse per-step backend whatever the target runs.
    ...
    def create_draft_extend_backend(self):
        if self._is_qwen_qsa_draft_model():
            ...
            # (QSA-wrapped hybrid) backend. Its replay path pads the variable
            # accepted-token rows to the captured static width, so the
            # draft-extend CUDA graph expresses the dynamic accept count.
