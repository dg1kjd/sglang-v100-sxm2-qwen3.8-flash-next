...
def dsv41_index_page_size() -> int:
    """Low-ratio indexer-K pool page. DeepGEMM wants 128; the torch path uses 64.

    SM70 never takes the DeepGEMM page size even if the env var is set.
    """
    from sglang.srt.runtime_context import get_platform

    if envs.SGLANG_DSV41_DEEP_GEMM_CANDIDATE_INDEXER.get() and (
        get_platform().is_sm90 or get_platform().is_sm100
    ):
        return 128
    return 64


# SM70 fail-closed default. Call dsv41_index_page_size() if the env/platform
# combination can still select 128 on SM90/SM100.
...
class DeepSeekV4TokenToKVPool(BaseSWAKVPool):
    ...
    def __init__(
        ...
        self.kv_source_layers = list(kv_source_layers)
        self.request_window = None
        if envs.SGLANG_ENABLE_DSV41_REQUEST_WINDOW.get():
            self._attach_dsv41_request_window(
                dtype=dtype,
                device=device,
                enable_memory_saver=enable_memory_saver,
                swa_page_size=swa_page_size,
                stage_layer_num=stage_layer_num,
                online_mtp_max_draft_tokens=online_mtp_max_draft_tokens,
            )

        # SM70 CSA2 never reads these pages. Without this, the hybrid
        # allocator bills SWA 1:1 with each prefill chunk, the 5120-token
        # floor fills, later chunks shrink, and a ~23k first prompt is
        # aborted by the client before it can be pinned.
        if (
            get_platform().is_sm70
            and self.swa_kv_pool is not None
            and self.swa_req_ring_size is None
        ):
            self.swa_req_ring_size = int(self.sliding_window or self.swa_page_size)
            logger.info(
                "SM70 CSA2 paged SWA is vestigial; per-request ring accounting "
                "ring_size=%d (chunked prefill is not charged 1:1)",
                self.swa_req_ring_size,
            )

    def _attach_dsv41_request_window(
        self,
        *,
        dtype,
        device,
        ...
        swa_page_size: int,
        stage_layer_num: int,
        online_mtp_max_draft_tokens: int,
    ) -> None:
        ...
        logger.info(
            "DSV4.1 RequestWindow attached: slots=%d layers=%d capacity=%d "
            "(paged SWA pool dropped)",
            self.num_req_slots,
            stage_layer_num,
            self.request_window.capacity,
        )
    ...
    def _init_paged_compress_states(self, enable_memory_saver: bool):
        ...
        for idx in range(self._stage_start, self._stage_end):
            ...
            # V4.1 ratios 1/2 keep pending/packed rows on the CSA2 path, not
            # the V4 Pro c4/c128 CompressStatePool.
            if ratio not in self.compressed_pool_configs:
                continue
            if ratio in (0, 1):
                continue
    ...
    def _init_compressed_layer_mapping(self):
        # 0 = uncompressed / SWA-only; 1/2 = V4.1 CSA2 (no c4/c128 pool);
        # 4/128 = V4 Pro compressed pools in kv_pools.
        layer_counts = {
            0: 0,
            1: 0,
            2: 0,
            **{ratio: 0 for ratio in self.kv_pools},
        }
        total_L = len(self.compression_ratios)
