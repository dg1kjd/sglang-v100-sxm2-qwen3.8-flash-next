...
def _require_sm90_deep_gemm():
    """Fail closed on SM70: never enter DeepGEMM arch checks."""
    from sglang.srt.runtime_context import get_platform

    platform = get_platform()
    if not (platform.is_sm90 or platform.is_sm100):
        raise RuntimeError(
            "DeepGEMM candidate indexer is SM90/SM100 only; SM70 uses "
            "TorchCandidateIndexer (SGLANG_DSV41_DEEP_GEMM_CANDIDATE_INDEXER ignored)."
        )
    import deep_gemm
...
def build_sparse_indexer_schedule(
    ...
    deep_gemm = _require_sm90_deep_gemm()
