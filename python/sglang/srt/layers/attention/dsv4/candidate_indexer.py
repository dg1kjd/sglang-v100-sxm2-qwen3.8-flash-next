...
def make_candidate_indexer(topk_blocks: int, block_size: int) -> CandidateIndexer:
    ...
    ``SGLANG_DSV41_DEEP_GEMM_CANDIDATE_INDEXER`` is set *and* the GPU is
    SM90/SM100. SM70 (and any other arch) fail closed to the torch algorithm.
    """
    ...
    if envs.SGLANG_DSV41_DEEP_GEMM_CANDIDATE_INDEXER.get() and (
        platform.is_sm90 or platform.is_sm100
    ):
